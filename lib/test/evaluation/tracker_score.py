...
def trackerlist(name: str, parameter_name: str, dataset_name: str, run_ids=None, display_name: str = None,
    ...
    if run_ids is None or isinstance(run_ids, int):
        run_ids = [run_ids]
    return [Tracker(name, parameter_name, dataset_name, run_id, display_name, result_only, params_dict) for run_id in
            run_ids]
...
class Tracker:
    ...
    def run_video(self, videofilepath, optional_box=None, debug=None, visdom_info=None, save_results=False,
        ...
        while True:
            ...
            if out['score'] < 0.85 and count >= 20:
                if back_times < 20:
                    tracker.last_state()
                    back_times += 1
                else:
                    tracker.rollback_state(n_frames=1)
                print('backing')
                count = 0

            if (last > 0.99 > out['score']) or (last < 0.99 < out['score']):
                # print(str(out['score']) + ' save')
                cache_info = tracker.save_cache(frame, state)
                tracker.save_state()
                count = 0
                back_times = 0

            if (last > 0.9 > out['score']) or (last < 0.9 < out['score']):
                # print(str(last) + ' ' + str(out['score']) + ' load')
                out = tracker.track(frame, cache_info)
                count += 1
            ...
            cv.putText(frame_disp, 'Press q to quit', (20, 80), cv.FONT_HERSHEY_COMPLEX_SMALL, 1,
                       font_color, 1)
            cv2.putText(frame_disp, f'FPS: {fps:.2f}', (20, 105), cv2.FONT_HERSHEY_COMPLEX_SMALL, 1, (255, 0, 0), 1)
