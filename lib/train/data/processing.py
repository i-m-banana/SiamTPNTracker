...
class SiamTPNProcessing(BaseProcessing):
    ...
    def __init__(self, search_area_factor, output_sz, center_jitter_factor, scale_jitter_factor,
        ...
        self.anchors = self._generate_anchors(settings.num, settings.factor, settings.bias)
        ...
        for s in ['search', 'template']:
            ...
            for ele in data[s + '_att']:
                ...
                if (mask_down == 1).all():
                    data['valid'] = False
                    return data                         
        data['template_anno'] = [anno * self.output_sz['template'] for anno in data['template_anno']]
        data['valid'] = True  
        centerness = prutils.centerness(data['search_anno'][0].view(-1, 4), self.anchors)
        data['centerness'] = [centerness]
        data['search_label'] = [(centerness>0.01).to(torch.long)]
        # Prepare output
